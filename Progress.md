...
## 范围
本轮新增总积分计划视角：按所有项目全集数汇总积分需求，按项目 staffing ratio 自动拆分到人员/岗位；新增岗位二级人员库、项目具体制作组名单、批次化积分发放/使用记录和人员额度汇总；成本曲线新增全局汇总/单项目分析切换；认证改为登录/注册门禁并按 Supabase 用户同步数据；暂不做月度拆分、不新增数据库结构。
...
- 已在 Zustand 本地持久化新增 `pointRecords`，用于保存积分记录；本轮未新增 Supabase 表字段。
...
- 积分批次记录目前保存在前端本地持久化状态，未进入 Supabase 云端同步；若需要多设备/多账号共享，需要先确认云端存储方案或新增列。
- 人员名称目前是自由文本输入，没有统一员工主数据；同一个人若写成不同名称会被视为不同人员。
- 具体人员库 `people` 目前同样只进入本地持久化，未进入 Supabase 云同步；同账号跨设备暂时不会同步具体人员和项目选人。
...
- 本轮不改 Supabase 数据库结构：先把记录系统放在本地持久化里，避免为了验证产品流程而扩大到数据库迁移。
